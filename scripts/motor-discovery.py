...
def generate_discovery_config(board: dict, mcu_serial: str, driver_type: str = "TMC2209") -> str:
    ...
    for port_name, port_config in motor_ports.items():
        ...
        if driver_type.startswith("TMC"):
            ...
            cs_pin = port_config.get("cs_pin")
            spi_bus = port_config.get("spi_bus")
            ...
            if tmc_type in ("tmc2209", "tmc2208", "tmc2225"):
                ...
                if cs_pin:
                    ...
                    if spi_bus:
                        config_lines.append(f"spi_bus: {spi_bus}")
                    config_lines.extend([
                        "run_current: 0.5",
                        "stealthchop_threshold: 999999",
                        "",
...
""")
        ...
        for port_name in self.motor_ports:
            ...
            if not self.buzz_motor(port_name):
                if not prompt_yes_no("Motor didn't respond. Continue anyway?"):
                    continue
